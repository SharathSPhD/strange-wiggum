...
def _task_breakdown(df: pd.DataFrame) -> str:
    has_combined = "combined" in df["condition"].unique()
    lines = ["## Per-Task Results\n"]
    if has_combined:
        header = "| Task | Category | Ralph μ | AttractorFlow μ | Combined μ | Winner |"
        sep =    "|------|----------|---------|-----------------|------------|--------|"
    else:
        header = "| Task | Category | Ralph μ | AttractorFlow μ | Winner |"
        sep =    "|------|----------|---------|-----------------|--------|"
    lines += [header, sep]
    ...
    for tid in sorted(df["task_id"].unique()):
        ...
        # Tie-aware best label
        sorted_conds = sorted(means, key=means.get, reverse=True)
        if len(sorted_conds) >= 2 and abs(means[sorted_conds[0]] - means[sorted_conds[1]]) < 0.01:
            best_lbl = "TIE"
        else:
            best_lbl = CONDITION_LABELS.get(sorted_conds[0], sorted_conds[0]) if sorted_conds else "—"

        if has_combined:
            c_mu = f"{means.get('combined', float('nan')):.2f}"
            lines.append(f"| {tid} | {cat} | {r_mu} | {a_mu} | {c_mu} | {best_lbl} |")
        else:
            lines.append(f"| {tid} | {cat} | {r_mu} | {a_mu} | {best_lbl} |")
...
def _methodology_footer(df: pd.DataFrame) -> str:
    n_tasks = df["task_id"].nunique()
    n_conds = df["condition"].nunique()
    n_reps = df["rep"].nunique()
    n_total = len(df)
    cond_str = " + ".join(
        CONDITION_LABELS.get(c, c) for c in sorted(df["condition"].unique())
    )
    alpha_adj = round(0.05 / max(1, n_conds * (n_conds - 1) // 2), 4)
    return f"""---
...
| Parameter | Value |
|-----------|-------|
| Design | Within-subjects ({n_conds} conditions per task) |
| Conditions | {cond_str} |
| Tasks | {n_tasks} |
| Reps | {n_reps} per task per condition |
...
| Post-hoc | Pairwise t-tests, Bonferroni α_adj = {alpha_adj} |
| Bootstrap | 10,000 iterations, seed = 42 |
| Effect sizes | Cohen's d (pairwise), partial η² (ANOVA) |
| Judge | Claude {MODEL} (blinded outputs) |
